...
// parseMBWOutputCustom 解析 mbw 输出
func parseMBWOutputCustom(output string) map[string]float64 {
...
func simpleMemoryTestCustom(language string) string {
	if EnableLoger {
	...
	results := parseMBWOutputCustom(string(output))
